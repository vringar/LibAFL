...
use libafl::{
    ...
    corpus::{CachedOnDiskCorpus, Corpus},
    events::SimpleRestartingEventManager,
    executors::{inprocess::InProcessExecutor, ExitKind, TimeoutExecutor},
    feedback_or,
    feedbacks::{AflMapFeedback, CrashFeedback, TimeFeedback},
...
fn fuzz(
    ...
    // CHECKME: is true, false correct here ???
    let map_feedback = AflMapFeedback::new_tracking(&edges_observer, true, false);
    ...
    let mut state = state.unwrap_or_else(|| {
        StdState::new(
            ...
            CachedOnDiskCorpus::new(corpus_dir, 1024).unwrap(),
            // Corpus in which we store solutions (crashes in this example),
            // on disk so the user can get them after stopping the fuzzer
            CachedOnDiskCorpus::new(objective_dir, 1024).unwrap(),
